...
$(function () {
  var _ks = [], _t = 1000 * 60, _vml = false, _lat = 23.569, _lng = 120.3038, _vm = null, _vp = null, _vz = null, _tr = null, _im = false, _vy = null;
  var $maps = $('#maps');
  ...
  function fmkm (m, a, n, cn, u, c) { if (u > cn) { m.setPosition (new google.maps.LatLng (m.getPosition ().lat () + a, m.getPosition ().lng () + n)); clearTimeout (window._fmkmTimer); window._fmkmTimer = setTimeout (function () { fmkm (m, a, n, cn + 1, u, c); }, 25); } else { if (c) c (m); } }
  ...
  function fmkg (m, w, c) { var n = m.getPosition (); var u = fgu (w, n); if (!u) return false; fmkm (m, u.a, u.n, 0, u.u, c); }
  function fmm (m, a, n, cn, u, c) { if (u > cn) { m.setCenter (new google.maps.LatLng (m.getCenter ().lat () + a, m.getCenter ().lng () + n)); clearTimeout (window.mmtr); window.mmtr = setTimeout (function () { fmm (m, a, n, cn + 1, u, c); }, 25); } else { if (c) c (m); } }
  ...
  function _flo (isFirst) {
    ...
    navigator.geolocation.getCurrentPosition (function (position) {
      if (isFirst === true) {
        ...
        _vy.setOptions ({ map: _vm, zIndex: _vz.zIndex + 1, position: t });
  ...
  function flp (i){
    ...
    _tr = setTimeout (function () {
      if (!_im && !(i % 10)) fmg (_vm, _p[i]);
      fmkg (_vz, _p[i], flp (i));
    }, 250);
  }

  function fsp (){
    _p = _ps[$select.val ()].map (function (t) { return new google.maps.LatLng (t[0] / 10 + 23, t[1] / 10 + 120); });
    _vp.setPath (_p);
    _vz.setPosition (_p[0]);
    setTimeout (flp, 1000);
  }
  function fin (){
    ...
    _vz = new OAML ({ map: _vm, position: p, icon: {path: 'M 0 0'}, labelAnchor: new google.maps.Point (40 / 2, 70), zIndex: 99999999, labelClass: 'mz', labelContent: '<img src="' + $maps.data ('icon') + '"/>' });
    _vy = new OAML ({ icon: {path: 'M 0 0'}, labelAnchor: new google.maps.Point (50 / 2, 50 / 2), zIndex: 0, labelClass: 'my', labelContent: '<img src="' + $maps.data ('avatar') + '"/>'});
