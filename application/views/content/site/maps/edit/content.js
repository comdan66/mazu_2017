...
$(function () {
  ...
  function fin (){
    ...
    $('#log').click (function () {
      console.error ('[' + _ps[$select.val ()].map (function (t) {
        return '[' + ((t[0] - 23) * 10).toFixed (5) + ',' + ((t[1] - 120) * 10).toFixed (5) +']';
      }).join (',') + ']');
      ;
    });

    _ps = $maps.data ('paths');
    for (var i in _ps) _ps[i] = _ps[i].map (function (t) { return [t[0] / 10 + 23, t[1] / 10 + 120]; })
