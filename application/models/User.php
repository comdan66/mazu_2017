...
class User extends OaModel {
  ...
  public static function avatar ($w = 100, $h = 100) {
    $size = array ();
    array_push ($size, isset ($w) && $w ? 'width=' . $w : ''); array_push ($size, isset ($h) && $h ? 'height=' . $h : '');
    
    if (User::current ())
      return 'https://graph.facebook.com/' . User::current ()->uid . '/picture' . (($size = implode ('&', array_filter ($size))) ? '?' . $size : '');
    else
      return  resource_url ('resource', 'image', 'user.png');
  }
}